...
function SingleProject() {
  ...
  useEffect (() => {
    const fetchProject = async () => {
      try {
        const {data} = await axios.get(`/workspaces/${workspaceId}/projects/${projectId}`)
  ...
  const fetchTasks = async () => {
    try {
      const {data} = await axios.get(`/workspaces/${workspaceId}/projects/${projectId}/tasks`, {
        params: {
          search,
          date: endDate,
          dateFrom,
  ...
  const fetchBurnOutGraph = async () => {
    try {
      const {data} = await axios.get(`/workspaces/${workspaceId}/projects/${projectId}/burn-out-graph`);
  ...
  const starProject = async (action:number) => {
    try {      
      ...
      await axios.post(`/workspaces/${workspaceId}/projects/${projectId}/star`, {
        action
      });
    } catch (error) {
      setProject([...project]);
  ...
  const deleteProject = async () => {
    try {
      const {data} = await axios.delete(`/workspaces/${workspaceId}/projects/${projectId}`);
      if (data.success) {
        navigate(`/workspaces/${workspaceId}`);
        dispatch(setAlert({message: 'Delete Was Successful!', type: 'success'}))
      }
    } catch (error) {
      console.log(error);
    }
    ...
    setDeleteDialog({
      ...
      onSuccess: async () => {
        try {
          ...
          const {data} = await axios.delete(`/workspaces/${workspaceId}/projects/${projectId}/tasks/${taskId}`);
  ...
  const updateStatus = async (e: any, taskId: string) => {
    try {
      ...
      const {data} = await axios.patch(`/workspaces/${workspaceId}/projects/${projectId}/tasks/${taskId}/status`, {
        status
      });
  ...
  const updatePriority = async (e: any, taskId: string) => {
    try {
      ...
      const {data} = await axios.patch(`/workspaces/${workspaceId}/projects/${projectId}/tasks/${taskId}/priority`, {
        priority
      });
  ...
  const handleDropDownClose = (e:any) => {
    if (e.target.id !== 'menuIcon' && !e.target.closest('.menu')
  ...
  const burnOutChartdata: ChartData<'line'> = {
    labels: burnoutData.map((data: any) => new Date(data.date).toLocaleDateString('en-GB', {month: 'short', day: 'numeric'})),
    datasets: [
        {
          label: 'Ideal Burnout',
          ...
          <div className="body">
            <div className="header">
              <div className='left'>
                ...
                <div className='right'>
                  <div onClick={toggleBurnOutGraph} className="chart-icon" style={{backgroundColor: showBurnDownGraph ? 'var(--color-violet)' : ''}}>
                    <i className="bi bi-bar-chart-line-fill"></i>
                  </div>
                  
                  <i onClick={(e) => starProject(project.starred ? 0 : 1)} 
                    title='Add to starred'
                    style={{color: project.starred ? 'yellow': 'var(--color-text-primary)', marginInline: '5%'}}
                    className={project.starred ? "bi bi-star-fill" : "bi bi-star"}
                  ></i>
                  <i 
                    onClick={()=> dropDown === 'delete' ? setDropDown('') : setDropDown('delete')} 
                    className="bi bi-three-dots-vertical" id='menuIcon' 
...

                  {
                    dropDown === 'delete' && (
                      <div className='dropdown-menu' style={{top: '100%', right: '0'}}>
                          <div onClick={handleEditProject} className={workspaceAdmin === userId ? 'dropdown-option' : 'dropdown-option disabled'}> 
                            <p>Edit</p>
                          </div>               
                          <div onClick={handleDeleteProject} className={workspaceAdmin === userId ? 'dropdown-option' : 'dropdown-option disabled'}>
                            <p>Delete Project</p>
                          </div>
                      </div>
                    )
            ...
            <div className='tasks'>
              {
                workspaceAdmin === userId && (
                  <div className='addTask'>
                    <button onClick={(e)=> openTask(e, '')} id='addTaskButton'>+ Add task</button>
                  </div>
                )
              }
              ...
              <div className='cards'>                
                <div className='task-status-wrapper'>
                  ...
                  tasks={[...tasks].filter((task: any) => task.status === 'Stuck')} 
                  openTask={openTask} 
                  handleDeleteTask={handleDeleteTask} 
                  updateStatus={updateStatus} updatePriority={updatePriority} 
                  />
                </div>
                <div className='task-status-wrapper'>
                  ...
                  <TaskCard 
                    projectTheme={project?.theme} 
                    tasks={[...tasks].filter((task: any) => task.status === 'Not Started')} 
                    openTask={openTask} 
                    handleDeleteTask={handleDeleteTask} 
                    updateStatus={updateStatus} updatePriority={updatePriority} 
                    />
